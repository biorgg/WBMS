<template>
  <Modal v-model="modalStatus"
         title="修改密码"
         :closable="updatePwdType === 0"
         :mask-closable="false"
         :footer-hide="true"
         @on-visible-change="reset">
    <div class="resetPwd">
      <h3 v-if="updatePwdType === 1"
          style="color:red;margin-bottom:10px">首次登陆请先修改密码</h3>
      <h3 v-if="updatePwdType === 2"
          style="color:red;margin-bottom:10px">已经超过1个月没修改过密码了哦~请修改密码后使用~</h3>
      <Form ref="resetPassWordForm"
        ...
        <FormItem prop="oldPwd"
                  label="原密码："
                  style="width:350px">
          <input-pass-word v-model="resetPassWordForm.oldPwd"
                           placeholder="请输入原密码"></input-pass-word>
        </FormItem>
        <FormItem prop="newPwd"
                  label="新密码："
                  style="width:350px">
          <input-pass-word v-model="resetPassWordForm.newPwd"
                           placeholder="6位以上,并且包含大小写字母和数字"></input-pass-word>
        </FormItem>
        <FormItem prop="secNewPwd"
                  label="重复新密码："
                  style="width:350px">
          <input-pass-word v-model="resetPassWordForm.secNewPwd"
                           placeholder="请再输入一次新密码"></input-pass-word>
        </FormItem>
        <FormItem style="width:350px">
          <Button style="margin-right:20px"
                  @click="_loginOut">退出登录</Button>
          <!--<Button style="margin-right:20px" @click="reset">重置</Button>-->
          <Button type="primary"
                  @click="submitPwd"
                  :loading="resetPassWordBtnLoading">提交</Button>
        </FormItem>
      </Form>
    </div>
  </Modal>
